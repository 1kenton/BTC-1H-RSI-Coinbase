"""
Main loop: every 60 seconds, fetch price data, evaluate RSI strategy, take trades.
...
def load_strategy(strategy_path: str = "state/strategy.yaml") -> dict:
    """Load strategy from YAML, support both relative and container paths"""
...
async def evaluate_rsi_strategy(price_data: dict, strategy: dict) -> dict:
    """
    Evaluate RSI < 30 entry, RSI > 70 exit.
...
async def loop_forever():
    ...
    while True:
        ...
        try:
            ...
            # Evaluate strategy
            trade = await evaluate_rsi_strategy(price_data, strategy)
...
if __name__ == "__main__":
    ...
    logger.info("Booting hermes-trading worker | asset=BTC/USDT | exchange=Coinbase")
    logger.info("  Mode: paper")
    logger.info("  Strategy: RSI < 30 entry / RSI > 70 exit")
    logger.info("")
